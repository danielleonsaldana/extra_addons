...
class PackageProduct(models.Model):
    _name = 'package.product'
    _description = 'Empaque'

    name = fields.Char(string='Empaque', required=True)
    ...
    def create(self, vals_list):
        ...
        return super(PackageProduct, self).create(vals_list)

    def write(self, vals):
        if 'xas_code' in vals:
            raise ValidationError("El campo código no es editable.")
        return super(PackageProduct, self).write(vals)
