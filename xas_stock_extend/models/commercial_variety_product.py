...
class CommercialVarietyProduct(models.Model):
    _name = 'commercial.variety.product'
    _description = 'Variedad Comercial'

    name = fields.Char(string='Variedad comercial', required=True)
    ...
    def create(self, vals_list):
        ...
        return super(CommercialVarietyProduct, self).create(vals_list)

    def write(self, vals):
        if 'xas_code' in vals:
            raise ValidationError("El campo código no es editable.")
        return super(CommercialVarietyProduct, self).write(vals)
