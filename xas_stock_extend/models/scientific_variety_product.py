...
class ScientificVarietyProduct(models.Model):
    _name = 'scientific.variety.product'
    _description = 'Variedad Científica'

    name = fields.Char(string='Variedad cientifica', required=True)
    ...
    def create(self, vals_list):
        ...
        return super(ScientificVarietyProduct, self).create(vals_list)

    def write(self, vals):
        if 'xas_code' in vals:
            raise ValidationError("El campo código no es editable.")
        return super(ScientificVarietyProduct, self).write(vals)
