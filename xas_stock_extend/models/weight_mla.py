...
class WeightMla(models.Model):
    _name = 'weight.mla'
    _description = 'Peso'

    name = fields.Char(string='Peso', required=True)
    ...
    xas_qty = fields.Float(string='Cantidad')
    xas_uom_qty =  fields.Many2one('uom.uom', string='Unidad de Medida')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('xas_code'):
                ...
                vals['xas_code'] = self._format_code(next_seq, 3)
        return super(WeightMla, self).create(vals_list)

    def write(self, vals):
        if 'xas_code' in vals:
            raise ValidationError("El campo código no es editable.")
        return super(WeightMla, self).write(vals)
